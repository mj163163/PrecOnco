...
pos = which(pathways$Pathways =="Genome integrity")
pathways = pathways[pos,]
...
final$col <- ifelse(final$DRUGS %in% c("Talazoparib"), "darkred", "ivory4")
final$shape <- ifelse(final$DRUGS == "Talazoparib",17,19) 
...
ggplot(final, aes(x=variable, y=value))+ 
  ...
  theme_classic(base_size = 20) + theme(axis.text.x = element_text(angle = 45, hjust=1,size=10),axis.text.y = element_text(size=10))+ylab("Prdicted Z-score (IC50) Genome Integrity")
